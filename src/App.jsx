...
import { ChevronLeft, ChevronRight, CheckSquare, Save, Loader2, DollarSign, Calculator, TrendingDown, LogIn, LogOut, User, Mail, Lock } from 'lucide-react';
...
export default function App() {
  ...
  const handleLogout = async () => {
    await signOut(auth);
    setWorkData({}); // Limpa dados da tela ao sair
  };
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-100 p-4 font-sans text-gray-800">
        <div className="bg-white p-8 rounded-xl shadow-xl w-full max-w-md border border-gray-200">
          ...
          <h2 className="text-2xl font-bold text-center mb-2 text-slate-800">
            {isRegistering ? 'Criar Conta' : 'Acesse seu Ponto'}
          </h2>
          <p className="text-center text-gray-500 mb-6 text-sm">
            {isRegistering ? 'Preencha para começar a controlar.' : 'Entre com seu e-mail e senha.'}
          </p>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-2 md:p-4 font-sans text-gray-800">
      <div className="max-w-5xl mx-auto bg-white shadow-xl rounded-xl overflow-hidden border border-gray-200">
        <div className="bg-slate-800 text-white p-6">
          <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 text-xs bg-slate-700/50 px-3 py-1.5 rounded-full border border-slate-600">
                <User className="w-3 h-3 text-slate-300" />
                <span className="text-slate-200 max-w-[150px] truncate">{user.email}</span>
                <button onClick={handleLogout} className="ml-2 hover:text-red-300 transition" title="Sair"><LogOut className="w-3 h-3"/></button>
              </div>
              <div className="flex items-center gap-2 bg-slate-700 rounded-lg p-1 shadow-inner">
                <button onClick={() => changeMonth(-1)} className="p-2 hover:bg-slate-600 rounded-md transition"><ChevronLeft className="w-5 h-5" /></button>
                <span className="font-semibold w-24 md:w-32 text-center text-sm uppercase tracking-wide">{monthNames[currentMonth]}</span>
                <button onClick={() => changeMonth(1)} className="p-2 hover:bg-slate-600 rounded-md transition"><ChevronRight className="w-5 h-5" /></button>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div className="bg-gradient-to-br from-green-900 to-green-800 p-3 rounded-lg border border-green-600 shadow-lg transform scale-105 flex flex-col justify-center relative overflow-hidden">
               ...
               <div className="flex justify-between w-full items-end z-10 mb-2 border-b border-green-700/50 pb-1">
                  <div className="flex items-center gap-1 text-red-300"><TrendingDown className="w-3 h-3" /><span className="text-[10px] uppercase font-bold tracking-wider">Desc. (11%)</span></div>
               ...
               <div className="flex flex-col w-full z-10 mt-1">
                  ...
                  <span className="text-2xl font-bold text-white leading-none">R$ {netEarnings.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
               </div>
            </div>
          </div>
        </div>
              ...
              {days.map((date) => {
                ...
                return (
                  <tr key={key} className={`hover:bg-blue-50 transition-colors ${isWeekend ? 'bg-orange-50/40' : 'bg-white'} ${isSunday ? 'border-b-4 border-gray-100' : ''}`}>
                    ...
                    <td className="p-3 text-center">{isWeekend ? (<div className="w-5 h-5 mx-auto rounded bg-gray-200 border border-gray-300 flex items-center justify-center opacity-50 cursor-not-allowed"><span className="block w-3 h-[1px] bg-gray-400 rotate-45"></span></div>) : (<input type="checkbox" checked={entry.worked} onChange={() => toggleDay(date, 'worked')} className="w-5 h-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500 cursor-pointer" />)}</td>
                    <td className="p-3 text-center">{isWeekend ? (<div className="w-5 h-5 mx-auto rounded bg-gray-200 border border-gray-300 flex items-center justify-center opacity-50 cursor-not-allowed"><span className="block w-3 h-[1px] bg-gray-400 rotate-45"></span></div>) : (<input type="checkbox" checked={entry.overtime} onChange={() => toggleDay(date, 'overtime')} className="w-5 h-5 rounded border-gray-300 text-yellow-500 focus:ring-yellow-500 cursor-pointer" />)}</td>
                    <td className="p-3 text-center">{isWeekend ? (<input type="checkbox" checked={entry.weekendWork} onChange={() => toggleDay(date, 'weekendWork')} className="w-5 h-5 rounded border-orange-300 text-orange-600 focus:ring-orange-500 cursor-pointer" />) : (<span className="text-gray-300">-</span>)}</td>
        ...
        <div className="p-3 bg-gray-50 border-t border-gray-200 flex justify-between items-center text-[10px] text-gray-400"><span>Valores aproximados.</span><span>ID: {user?.uid?.slice(0, 6)}</span></div>
      </div>
    </div>
  );
}